---
title: "Let's all Support Vector Machines"
...
1.  **Choose a Kernel:** Instead of trying to find a complicated mapping function $\phi$, we simply choose a kernel function $K$. Two popular examples are:
    ...
    * **Radial Basis Function (RBF) Kernel:** $K(\mathbf{x}_i, \mathbf{x}_j) = \exp(-\gamma \|\mathbf{x}_i - \mathbf{x}_j\|^2)$. This is the most popular choice. It corresponds to a feature mapping into an **infinite-dimensional** space, which would be impossible to compute the "hard way."
...
3.  For the RBF kernel, $K(\mathbf{x}, \mathbf{x}) = \exp(-\gamma \|\mathbf{x} - \mathbf{x}\|^2)$.
4.  Since $\|\mathbf{x} - \mathbf{x}\|^2$ is always zero for *any* vector $\mathbf{x}$, the expression simplifies to $\exp(0) = 1$.

This proves that $\|\phi(\mathbf{x})\|^2 = 1$ for every point. The specific coordinates of $\mathbf{x}$ cancel themselves out, making the result universal for all points.
...
* **Points close in original space:** If $\mathbf{x}_i$ and $\mathbf{x}_j$ are close, the term $\|\mathbf{x}_i - \mathbf{x}_j\|^2$ is small, making $K(\mathbf{x}_i, \mathbf{x}_j) = \exp(-\text{small value})$ close to 1. This means they are mapped to points that are very **close to each other** on the hypersphere.
* **Points far apart in original space:** If $\mathbf{x}_i$ and $\mathbf{x}_j$ are far apart, the term $\|\mathbf{x}_i - \mathbf{x}_j\|^2$ is large, making $K(\mathbf{x}_i, \mathbf{x}_j) = \exp(-\text{large value})$ close to 0. This means they are mapped to points that are **far from each other** (nearly orthogonal) on the hypersphere.
