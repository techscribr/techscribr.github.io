...
The covariance between two random variables, $X$ and $Y$, is defined as the expected value of the product of their deviations from their respective means:

$$
\text{Cov}(X, Y) = E[(X - E(X))(Y - E(Y))]
$$

Before calculating the covariance matrix, data is typically **centered**. Centering data $\mathbf{x}$ involves subtracting the mean $\mu$ of the entire dataset $\mathbf{X}$ from every data point: $\mathbf{X}_{\text{centered}} = \mathbf{X} - E(\mathbf{X})$.
...
The expression to maximize, $\|\mathbf{X}\mathbf{w}\|^2$, can be reformulated using properties of vectors and matrices.
...
The problem of maximizing the quadratic form $\mathbf{w}^T \mathbf{C} \mathbf{w}$ subject to $\|\mathbf{w}\|^2 = 1$ is a standard problem solved by **Eigen Decomposition**.
